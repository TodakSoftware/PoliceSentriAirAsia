...
public class Robber : MonoBehaviourPunCallbacks
{
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            if (other.gameObject.GetComponent<Robber>().isCaught == false)
            { // our team mates who not caught
    ...
    [PunRPC]
    public void DisplayMoneybag(bool show)
    { // Called by Moneybag (OnTriggerEnter2D)
        if (show)
        {
    ...
    [PunRPC]
    public IEnumerator InvulnerableEffect(float duration)
    {  // Used by robber against police
       //if(photonView.IsMine){
